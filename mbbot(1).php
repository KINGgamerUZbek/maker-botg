<?php
/*Ushbu kod: Joliddinov Umidjon(https://t.me/NET_CODER_UZ) tomonidan yozilgan. Iltimos, mualliflik huquqi hurmat qilinsin!*/
ob_start();
...
function phonenumber($id){
    ...
    "chat_id"=>$id,
"photo"=>"",
    "text"=>"<b>Salom, hurmatli foydalanuvchi!</b>\n<b>Mb ishlash ishonchli bo'lishi uchun, pastdagi «📲 Telefon raqamni yuborish» tugmasini bosing:</b>",
    "parse_mode"=>"html",
    "reply_markup"=>json_encode([
...
function reyting(){
    $text = "🏆 <b>TOP 10 ta eng koʻp Mb ishlagan foydalanuvchilar:</b>\n\n";
    $daten = [];
    $rev = [];
    $fayllar = glob("./uzfox/*.*");
    ...
    for($i=0;$i<10;$i+=1){
        $order = $i+1;
        $id = $daten["$reversed[$i]"];
        $text.= "<b>{$order}</b>.{$id}- "."<code>".$reversed[$i]."</code>"." <b>soʻm</b>"."\n";
    }
    return $text;
}
...
"resize_keyboard"=>true,
    "keyboard"=>[
[["text"=>"💸 MB ishlash"],["text"=>"💳 Hisobim"],],
[["text"=>"🗒 Qo‘llanma"],["text"=>"🏆 Reyting"],],
[["text"=>"✉ Administratsiya"],["text"=>"📊 Statistika"],],
]
]);
...
uzfox("sendMessage",[
"chat_id"=>$referalid,
"text"=>"<b>👏 Tabriklaymiz! Sizni do'stingiz</b> <a href='tg://user?id=$fromid'>$firstname</a> <b>botimizdan ro'yxatdan o'tdi va sizga $referalsum Mb taqdim etildi.</b>",
"parse_mode"=>"html",
"reply_markup"=>$menu,
]);
}
...
typing($chatid);
if($text=="🗣 Userlarga xabar yuborish" or $text=="👥 Referal narxini o'zgartirish" or $text=="💳 Hisob tekshirish" or $text=="💰 Hisob toʻldirish" or $text=="✅ Bandan olish" or $text=="🚫 Ban berish" or $text=="📤 Minimal Mb yechish" or $text=="⬅️ Ortga"){
}else{
...
uzfox("sendMessage",[
"chat_id"=>$admin,
"text"=>"<b>Foydalanuvchi hisobini necha mb ga toʻldirmoqchisiz:</b>",
"parse_mode"=>"html",
"reply_markup"=>$panel,
]);
}
...
uzfox("sendMessage",[
   "chat_id"=>$text,
          "text"=>"💰 Hisobingiz: $summa Mb ga to'ldirildi!\nHozirgi hisobingiz: $jami",
]);
uzfox("sendMessage",[
"chat_id"=>$admin,
...
uzfox("sendMessage",[
"chat_id"=>$admin,
"text"=>"<b>Referal taklif qilish narxi: $text Uc ga o'zgardi!</b>",
"parse_mode"=>"html",
"reply_markup"=>$panel,
]);
unlink("uzfoxbot/$chatid.step");
...
uzfox("sendMessage",[
"chat_id"=>$admin,
"text"=>"<b>Minimal Mb yechish narxini kiriting:</b>",
"parse_mode"=>"html",
"reply_markup"=>$panel,
]);
}
...
uzfox("sendMessage",[
"chat_id"=>$admin,
"text"=>"<b>Minimal Mb yechish narxi: $text Mb ga o'zgardi!</b>",
"parse_mode"=>"html",
"reply_markup"=>$panel,
]);
unlink("uzfoxbot/$chatid.step");
...
if($text=="💸 MB ishlash" and $ban==false){
...
uzfox("sendMessage",[
    "chat_id"=>$chatid,
"photo"=>"https://t.me/MaxzuN_YuraklaR09/7",
    "text"=>"<b>Hurmatli $firstname botimizdan Mb ishlash uchun pastdagi referal havolani do‘stlaringizga ulashing va pul ishlang!</b>
...
"reply_markup"=>json_encode([
"inline_keyboard"=>[
[["text"=>"💰Mb ishlashni boshlash","switch_inline_query"=>$chatid],],
]
]),
]);
}
...
"chat_id"=>$chatid,
"photo"=>"",
"text"=>"<b>Sizning balansingiz:</b> $sum <b>Mb</b>
<b>Taklif qilgan do‘stlaringiz:</b> $referal <b>ta</b>
<b>Sizning raqamingiz:</b> +$contact

<b>Mb yechib olish uchun minimal Mb:</b> <code>$minimalsumma</code> <b>Mb</b>",
"parse_mode"=>"html",
"reply_markup"=>json_encode([
"inline_keyboard"=>[
[["text"=>"📤 Mb yechish","callback_data"=>"production"],],
]
]),
]);
}
...
✅<b>Bot to‘lab bergan jami summa: $jami Mb</b>",
"parse_mode"=>"html",
"reply_markup"=>$menu,
]);
}
...
if($sumid>=$minimalsumma){
 ...
 uzfox("sendMessage",[
    "chat_id"=>$chat_id,
          "text"=>"💰 <b>Sizning hisobingizda: $sumid Mb mavjud!</b>\n<b>Pulingizni yechib olish uchun hamyonlarni birini tanlang!</b>",
          "parse_mode"=>"html",
          "reply_markup"=>json_encode([
              "inline_keyboard"=>[
                  [["text"=>"Telefon Mb","callback_data"=>"paynet"],],
          ...
          uzfox("answerCallbackquery",[
              "callback_query_id"=>$id,
              "text"=>"☝️ Sizning hisobingizda mablag' yetarli emas!\nSizga yana mablag'ni yechib olish uchun $som Mb kerak!\nSizning hisobingizda: $sumid Mb mavjud!",
              "show_alert"=>true,
]);
}
}
...
if($sumid>=$minimalsumma){
 ...
 uzfox("sendMessage",[
    "chat_id"=>$chat_id,
              "text"=>"❗️ Mobil telefon raqamini KIRITING!\nNa'muna: 998941111111",
          "reply_markup"=>json_encode([
             "one_time_keyboard"=>true,
"resize_keyboard"=>true,
    "keyboard"=>[
...
stepbot($chatid,"summa");
              uzfox("sendMessage",[
                  "chat_id"=>$chatid,
                  "text"=>"💳 To'lov miqdorini kiriting.\n💰 Sizning hisobingizda: $hisob Mb mavud!",
"reply_markup"=>json_encode([
             "one_time_keyboard"=>true,
"resize_keyboard"=>true,
    "keyboard"=>[
...
file_put_contents("uzfox/$chatid.raqam","$text");
          ...
          uzfox("sendMessage",[
              "chat_id"=>$chatid,
              "text"=>"❗️Telefon raqamini KIRITING!\nNa'muna: 998941111111",
              ]);
}
}
...
"parse_mode"=>"markdown",
       ...
       uzfox("sendMessage",[
              ...
              "parse_mode"=>"html",
               "reply_markup"=>json_encode([   
   "inline_keyboard"=>[
[["text"=>"🤖 Bot ochish","url"=>"https://t.me/MakerUz_konstruktor_bot"],],
]
]),
]);
}
...
"chat_id"=>$chatid,
"photo"=>"",
"text"=>"<b>Savol - botda qanday qilib MB  ishlash mumkin?

Javob - botda MB  ishlash juda oson, MB  ishlash tugmasini bosing. Sizga berilgan unikal-havolani doʻstlaringizga yuboring. Doʻstingiz siz tarqatgan havola orqali botga kirib, bot bergan kanallarga a'zo bo‘lsa, biz sizning bot hisobingizga 0 MB  oʻtkazamiz.

Qanday qilib MB  botdan chiqarish mumkin? MB larni chiqarish to'g'ridan-to'g'ri sizning mobil Telefoningizga otqaziladi.</b>",
"parse_mode"=>"html",
"reply_markup"=>$menu,
]);
}
...
if($text=="✉ Administratsiya" and $ban==false){
if((joinchat($fromid)=="true") and (phonenumber($fromid)=="true") and ($ban==false)){
uzfox("sendMessage",[
   "chat_id"=>$chatid,
   "text"=>"<b>Admin bilan bog‘lanish!</b>

<i>Admin bilan bog‘lanish uchun pastagi 👤 Admin tugmasini bosing biz bilan faqat to‘lovlar va reklama haqida yozishingiz mumkin botdan tashqari savollar berish bloklanishga olib keladi!</i>

<b>Murojaat qilish: @$admin_user</b>",
"parse_mode"=>"html",
"reply_markup"=>json_encode([
"inline_keyboard"=>[
[["text"=>"👤 Admin","url"=>"https://t.me/$admin_user"],['text'=>"🛠️Bot yaratish","url"=>"https://t.me/MakerUz_konstruktor_bot"],],
]
]),
]);
}
