...
if($text == "/start"){
	if(! in_array($from_id, $users)){
		...
		bot('sendMessage',[
		'chat_id' => $from_id,
		'text' => "✌️Salom aziz $name  🤖Mening vazifam .zip fayl yaratish.
🗂️.zip fayl yaratish uchun quyidagi menudan foydalaning.
🎧Bizning kanalimiz @$channel -ga ham obuna bo'ling",
	...
	elseif($text == "🗄Fayl yaratish"){
		...
		bot('sendMessage', [
		'chat_id' => $from_id,
		'text' => "🎟️.zip fayl yaratish uchun faylga nom bering ohiriga .zip kengaytmasini qo'shgan xolda.
Ⓜ️asalan: Maylime.Zip",
		'parse_mode'=>"html",
'reply_markup' => json_encode(['keyboard' => [
		[['text' => "🔙"]]
...
🔸Tayyor tugmasini bosing",
 ...
 🕐 bir oz kuting..." 
			...
			'caption' => "☑️Fayl tayyor bo'ldi.
🎧Music Kanal: @iSearchMe_Bot .
🗂️Php kodlar kanali @EditUz_Bot",
			'reply_markup' => json_encode(['keyboard' => [
			[['text' => "🗄Fayl yaratish"]]
			], 'resize_keyboard' => true
			])
