...
$botname = uzfox('getme',['bot'])->result->username;
     ...
     if(file_exists("uzfox/".$id.".referalid")){
           ...
           if($get=="true"){
            ...
            file_put_contents("uzfox/".$file.".pul","$user");
             uzfox("sendMessage",[
             "chat_id"=>$file,
             "text"=>"<b>Sizning do'stingiz</b>, <a href='tg://user?id=".$id."'>".$firstname."</a> <b>bizning kanallarimizdan chiqib ketgani uchun sizga ".$minimal." rubl jarima berildi.</b>",
             "parse_mode"=>"html",
             "reply_markup"=>$menu,
             ]);
           }
...
function phonenumber($id){
    ...
    "chat_id"=>$id,
"photo"=>"",
    "text"=>"<b>Salom, hurmatli foydalanuvchi!</b>\n<b>Rubl ishlash ishonchli bo'lishi uchun, pastdagi «📲 Telefon raqamni yuborish» tugmasini bosing:</b>",
    "parse_mode"=>"html",
    "reply_markup"=>json_encode([
...
function reyting(){
    $text = "🏆 <b>TOP 10 ta eng koʻp rubl ishlagan foydalanuvchilar:</b>\n\n";
    $daten = [];
    $rev = [];
    $fayllar = glob("./uzfox/*.*");
    ...
    for($i=0;$i<10;$i+=1){
        $order = $i+1;
        $id = $daten["$reversed[$i]"];
        $text.= "<b>{$order}</b>.{$id}- "."<code>".$reversed[$i]."</code>"." <b>rubl</b>"."\n";
    }
    return $text;
}
...
}else{    file_put_contents("uzfox/minimal.sum","1000");
}
...
"resize_keyboard"=>true,
    "keyboard"=>[
[["text"=>"💶 Rubl ishlash"],["text"=>"📱 Kabinet"],],
[["text"=>"🗒 Qo‘llanma"],],
[["text"=>"🏆 Reyting"],["text"=>"📊 Statistika"],],
...
"resize_keyboard"=>true,
    "keyboard"=>[
[["text"=>"1-kanal"],["text"=>"2-kanal"],["text"=>"3-kanal"],],
[["text"=>"⬅ Back"],],
]
]);
...
"resize_keyboard"=>true,
    "keyboard"=>[
[["text"=>"📱 Hisob parametrlari"],],
[["text"=>"🛠 Kanal parametrlari"],["text"=>"💶 Rubl parametrlari"],],
[["text"=>"🔐 Blok parametrlari"],["text"=>"📨 Barchaga xabar yuborish"],],
[["text"=>"⬅ Ortga"],],
]
]);
...
"resize_keyboard"=>true,
"keyboard"=>[
[["text"=>"📱 Hisobni tekshirish"],["text"=>"💳 Hisob to'ldirish"],],
[["text"=>"⬅ Back"],],
]
]);

$kanal_panel = json_encode([
"resize_keyboard"=>true,
"keyboard"=>[
[["text"=>"🛠 Majburiy obuna"],["text"=>"🛠 To'lovlar kanali"],],
[["text"=>"⬅ Back"],],
]
]);

$pul_panel = json_encode([
"resize_keyboard"=>true,
"keyboard"=>[
[["text"=>"🔗 Referal narxi"],["text"=>"🔐 Minimal rubl yechish"],],
[["text"=>"⬅ Back"],],
]
]);

$blok_panel = json_encode([
"resize_keyboard"=>true,
"keyboard"=>[
...
if($text=="/panel" and $chatid==$admin){
typing($chatid);
uzfox('sendMessage',[
"chat_id"=>$chatid,
...
if($text=="🗄 Boshqaruv paneli 🛠" and $chatid==$admin){
typing($chatid);
uzfox('sendMessage',[
"chat_id"=>$chatid,
"text"=>"<b>Salom, Siz bot administratorisiz. Kerakli boʻlimni tanlang:</b>",
...
if($text=="📱 Hisob parametrlari" and $chatid==$admin){
typing($chatid);
uzfox('sendMessage',[
"chat_id"=>$chatid,
"text"=>"<b>🛠 Hisob parametrlari</b>",
"parse_mode"=>"html",
...
if($text=="🛠 Kanal parametrlari" and $chatid==$admin){
	typing($chatid);
uzfox('sendMessage',[
"chat_id"=>$chatid,
"text"=>"<b>🛠 Kanal parametrlari</b>",
"parse_mode"=>"html",
"reply_markup"=>$kanal_panel,
]);
}

if($text=="💶 Rubl parametrlari" and $chatid==$admin){
	typing($chatid);
uzfox('sendMessage',[
"chat_id"=>$chatid,
"text"=>"<b>💶 Rubl parametrlari</b>",
"parse_mode"=>"html",
"reply_markup"=>$pul_panel,
]);
}

if($text=="🔐 Blok parametrlari" and $chatid==$admin){
	typing($chatid);
uzfox('sendMessage',[
"chat_id"=>$chatid,
"text"=>"<b>🔐 Blok parametrlari</b>",
"parse_mode"=>"html",
"reply_markup"=>$blok_panel,
]);
}
...
uzfox('sendMessage',[
"chat_id"=>$chatid,
"text"=>"<b>To'lovlar kanalini o'zgartirish uchun 
...
if($text=="🛠 Majburiy obuna" and $chatid==$admin){
typing($chatid);
uzfox('sendMessage',[
"chat_id"=>$chatid,
...
if($text=="📨 Barchaga xabar yuborish" and $chatid==$admin){
typing($chatid);
...
<pre>[uzfox+https://t.me/iVipBuilder]\n[Yangiliklar+https://t.me/By_Developers]</pre>",//qoy
"parse_mode"=>"html",
                      "disable_web_page_preview"=>true,
                    ]);
...
if($text=="📱 Hisobni tekshirish" and $chatid==$admin){
typing($chatid);
...
typing($chatid);
if($text=="📨 Barchaga xabar yuborish" or $text=="🔗 Referal narxi" or $text=="📱 Hisobni tekshirish" or $text=="💳 Hisob to'ldirish" or $text=="🔐 Blokdan olish" or $text=="🔒 Bloklash" or $text=="📤 Minimal rubl yechish" or $text=="⬅ Orqaga"){
}else{
...
if($text=="💳 Hisob to'ldirish" and $chatid==$admin){
typing($chatid);
stepbot($chatid,"coin");
uzfox("sendMessage",[
"chat_id"=>$admin,
...
if($text=="🔗 Referal narxi" and $chatid==$admin){
typing($chatid);
...
if($text=="🔐 Blokdan olish" and $chatid==$admin){
...
if($text=="🔒 Bloklash" and $chatid==$admin){
...
if($text=="🔐 Minimal rubl yechish" and $chatid==$admin){
typing($chatid);
stepbot($chatid,"minimalsumma");
uzfox("sendMessage",[
"chat_id"=>$admin,
"text"=>"<b>Minimal rubl yechish narxini kiriting:</b>",
"parse_mode"=>"html",
"reply_markup"=>$panel,
]);
}
...
if($text=="💶 Rubl ishlash" and $ban==false){
...
uzfox("sendMessage",[
    "chat_id"=>$chatid,
"photo"=>"",
    "text"=>"<b>Hurmatli $firstname botimizdan Rubl ishlash uchun pastdagi referal havolani do‘stlaringizga ulashing va Rubl ishlang!</b>
...
"reply_markup"=>json_encode([
"inline_keyboard"=>[
[["text"=>"💶 Rubl ishlashni boshlash","switch_inline_query"=>$chatid],],
]
]),
]);
}
...
"chat_id"=>$chatid,
"photo"=>"",
"text"=>"<b>Sizning balansingiz:</b> $sum <b>so‘m</b>
<b>Taklif qilgan do‘stlaringiz:</b> $referal <b>ta</b>
<b>Sizning raqamingiz:</b> +$contact

<b>Rubl yechib olish uchun minimal summa:</b> <code>$minimalsumma</code> <b>soʻm</b>",
"parse_mode"=>"html",
"reply_markup"=>json_encode([
"inline_keyboard"=>[
[["text"=>"📤 Rubl yechish","callback_data"=>"production"],],
]
]),
]);
}
...
✅<b>Bot to‘lab bergan jami summa: $jami rubl </b>",
"parse_mode"=>"html",
"reply_markup"=>$menu,
]);
}
...
if($sumid>=$minimalsumma){
 ...
 uzfox("sendMessage",[
    "chat_id"=>$chat_id,
          "text"=>"💰 <b>Sizning hisobingizda: $sumid rubl mavjud!</b>\n<b>rublingizni yechib olish uchun hamyonlarni birini tanlang!</b>",
          "parse_mode"=>"html",
          "reply_markup"=>json_encode([
              "inline_keyboard"=>[
...
uzfox("sendmessage",[
"chat_id"=>$chatid,
            "text"=>"💵 Sizning hisobingizda siz yechib olmoqchi bo'lgan rubl mavjud emas!\nSiz faqat $hisob rubl  yechib olishingiz mumkin!",
          ]);
unlink("uzfoxbot/$chatid.step");
}
}
...
if((stripos($callbackdata,"send|")!==false) and ($from_id=="ADMIN_ID")){
    uzfox("deleteMessage",[
    "chat_id"=>$chat_id,
    "message_id"=>$message_id,
]); 
...
if((stripos($callbackdata,"no|")!==false) and ($from_id=="ADMIN_ID")){
        uzfox("deleteMessage",[
    "chat_id"=>$chat_id,
    "message_id"=>$message_id,
]); 
       ...
       uzfox("sendMessage",[
              "chat_id"=>$id,
              "text"=>"<b>Assalom-u alaykum, $name!</b>\n<b>Sizning botdan yechib olgan rublingiz bekor qilindi!</b>",
              "parse_mode"=>"html",
               "reply_markup"=>$menu,
]);
}

if((stripos($callbackdata,"ban|")!==false) and ($from_id=="ADMIN_ID")){
        uzfox("deleteMessage",[
    "chat_id"=>$chat_id,
    "message_id"=>$message_id,
]); 
...
"chat_id"=>$chatid,
"photo"=>"",
"text"=>"<b>Savol - botda qanday qilib rubl ishlash mumkin?</b>\n\n<b>Javob - botda rubl ishlash juda oson, rubl ishlash tugmasini bosing. Sizga berilgan unikal-havolani doʻstlaringizga yuboring. Doʻstingiz siz tarqatgan havola orqali botga kirib, bot bergan kanallarga a'zo bo‘lsa, biz sizning bot hisobingizga $referalsum soʻm oʻtkazamiz.</b>\n\n<b>Qanday qilib  botdan chiqarish mumkin? rubllarni chiqarish to'g'ridan-to'g'ri sizning mobil telefoningizning hisobiga yoki kartangiz hisobiga amalga oshiriladi:
...
<b>Botdagi a‘zolar soni:</b> $member <b>ta</b>
